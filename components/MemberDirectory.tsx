...
export const MemberDirectory: React.FC<MemberDirectoryProps> = ({ 
  ...
  const checkArrears = (memberId: string) => {
    ...
    return memberActiveLoans.some(activeLoan => {
    ...
    >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredMembers.map(member => {
          ...
          const hasActiveLoans = memberActiveLoans.length > 0;
              ...
              <div className="w-full bg-paper-100/50 rounded-sm p-4 mb-6 border border-paper-200 space-y-4">
                ...
                {hasActiveLoans && (
                  <div className="pt-3 border-t border-paper-200">
                     <div className="text-[10px] font-black uppercase text-ink-400 tracking-widest mb-1 flex items-center justify-center gap-1"><CreditCard size={12} className="text-blue-600" /><span>Active Portfolio</span></div>
                     <div className="flex flex-col items-center">
                        <div className="text-lg font-mono font-bold text-blue-800">₱{totalActivePrincipal.toLocaleString()}</div>
                        <div className="text-[10px] font-mono text-ink-400 uppercase tracking-tighter">{memberActiveLoans.length > 1 ? `${memberActiveLoans.length} Loans` : 'Remaining Principal'}</div>
                     </div>
                  </div>
                )}
              </div>
